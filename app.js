...
app.post("/forum", upload, (req, res) => { //posting a new thread, this should be a protected view visible only after login
    ...
    var url = "mongodb+srv://mrbacco:mongodb001@cluster0-goutv.mongodb.net/users.threads?retryWrites=true";
    ...
    MongoClient.connect(url, function(err, db) {
        ...
        upload(req, res, (err) => { //calling the upload method define before
            if (err) {
                ...
                newThread.img.data = fs.readFileSync(req.file.path, (err) => {
                    if (err) throw err;
                        newThread.img.contentType = 'image/jpeg';
                        dbo.collection("threads").insertOne(newThread, function(err, i) {
                            if (err) {
                                console.log(err + " what error is sthis");
                                return;
                            } else {
                                console.log("new thread " + newThread.title + " has been created");
                            }
